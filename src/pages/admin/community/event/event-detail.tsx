...
import { getEventById, deleteEvent } from "@/lib/api/event";
...
export function EventDetail({ language }: EventDetailProps) {
  ...
  const handleEdit = () => {
    if (id) {
      navigate(`/admin/community/event/${id}`);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (response.success) {
      ...
      toast.error(response.message || "이벤트 삭제에 실패했습니다");
    }
  };
  ...
  if (!post) {
    return (
      <div className="space-y-6">
        <PageHeader
          title={language === "ko" ? "이벤트 상세" : "Event Detail"}
          subtitle={language === "ko" ? "이벤트 상세 정보를 확인하세요" : "View event details"}
          language={language}
        />
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 내용 */}
      <div className="bg-card rounded-[8px] border p-4 sm:p-6">
        <h3 className="text-[15px] mb-4">{language === "ko" ? "이벤트 내용" : "Event Content"}</h3>
        <div 
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {language === "ko" ? "이벤트 삭제" : "Delete Event"}
            </AlertDialogTitle>
