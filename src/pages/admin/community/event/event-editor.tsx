...
import { getEventById, createEvent, updateEvent } from "@/lib/api/event";
import { 
  ArrowLeft, 
  Save, 
  X, 
...
export function EventEditor({ language }: EventEditorProps) {
  ...
  const [eventEndDate, setEventEndDate] = useState(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));
    ...
    if (isEditMode) {
      getEventById(id).then(response => {
        if (response.success && response.data) {
          ...
          setEventEndDate(new Date(post.eventEndDate));
    ...
    try {
      if (isEditMode) {
        ...
        const response = await updateEvent({
          ...
          eventEndDate: `${eventEndDate.getFullYear()}-${String(eventEndDate.getMonth() + 1).padStart(2, '0')}-${String(eventEndDate.getDate()).padStart(2, '0')}`,
          views: 0,
          isActive: true,
        });
        ...
        if (response.success) {
          ...
          toast.error(response.message || "이벤트 수정에 실패했습니다");
        }
      } else {
        // 
        ...
        if (response.success) {
          ...
          toast.error(response.message || "이벤트 등록에 실패했습니다");
        }
      }
    } catch (error) {
  ...
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative w-full h-full overflow-auto">
      ...
      <PageHeader
        title={isEditMode 
          ? (language === "ko" ? "이벤트 수정" : "Edit Event")
          : (language === "ko" ? "이벤트 작성" : "Write Event")
        }
      ...
      <div className="bg-card relative rounded-[8px] flex-1 flex flex-col w-full">
        ...
        <div className="box-border content-stretch flex flex-col gap-[24px] items-start p-[24px] flex-1 overflow-auto">
          ...
          <div className="w-full space-y-2">
            <Label className="text-[13px]">
              {language === "ko" ? "이벤트 종료일" : "Event End Date"}
            </Label>
            <DatePicker
              value={eventEndDate ? `${eventEndDate.getFullYear()}-${String(eventEndDate.getMonth() + 1).padStart(2, '0')}-${String(eventEndDate.getDate()).padStart(2, '0')}` : ''}
              onChange={(dateStr) => setEventEndDate(new Date(dateStr + 'T00:00:00'))}
            />
          </div>
